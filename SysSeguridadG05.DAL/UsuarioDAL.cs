...
namespace SysSeguridadG05.DAL
{
    ...
    {
        private static void EnciptarMD5(Usuario pUsuario)
        {
        ...
        public static async Task<int> GuardarAsync(Usuario pUsuario)
        {
            ...
            try
            {
                ...
                {
                    bool existenceLogin = await ExisteLogin(pUsuario, dbContexto);
                    if(existenceLogin == false)
                    {
                        pUsuario.FechaRegistro = DateTime.Now;
                        EnciptarMD5(pUsuario);
        ...
        public static async Task<int> ModificarAsync(Usuario pUsuario)
        {
            ...
            try
            {
                using(var dbContexto = new DBContexto())
                {
                    ...
                    {
                        throw new Exception("Login ya existente");
                    }
                }
            }
            catch ( Exception ex)
        ...
        internal static IQueryable<Usuario> QuerySelect(IQueryable<Usuario> pQuery, Usuario pUsuario)
        {
            ...
            if (!string.IsNullOrWhiteSpace(pUsuario.Apellido))
                pQuery = pQuery.Where(s => s.Apellido == pUsuario.Apellido);
        ...
        public static async Task<Usuario> LoginAsync(Usuario pUsuario)
        {
            ...
            try
            {
                ...
                {
                    EnciptarMD5(pUsuario);
                    usuario = await dbContext.Usuario.FirstOrDefaultAsync(s => s.Login == pUsuario.Login && s.Password == pUsuario.Password && s.Estatus == (byte)Estatus_Usuario.Activo);
                }
            }
            catch (Exception ex)
            {
        ...
        public static async Task<int> CambiarPasswordAsync(Usuario pUsuario, string pPasswordAnt)
        {
            ...
            using (var dbContexto = new DBContexto())
            {
                ...
                {
                    throw new Exception("La password actual es incorrecta");
                }
            }
            return result;
